 ...
 * under the License.
 */
import { CategoricalColorNamespace, convertMetric, getNumberFormatter } from '@superset-ui/core';
...
export default function transformProps(chartProps) {
  ...
  const numberFormatter = getNumberFormatter(numberFormat); // TODO: remove cast once metrics is cleaned up in QueryFormData
  ...
  const outlierData = data.map(datum => {
    return metrics.map(metric => {
      const groupbyLabel = extractGroupbyLabel({
        datum,
        groupby
      });
      const name = metrics.length === 1 ? groupbyLabel : `${groupbyLabel}, ${metric}`; // Outlier data is a nested array of numbers (uncommon, therefore no need to add to DataRecordValue)
